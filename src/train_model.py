...
def train():
    try:
        ...
        model_college = RandomForestClassifier(n_estimators=100, max_depth=20, random_state=42, n_jobs=-1)
        model_college.fit(X, y_college)

        print("🎓 College model trained!")

        print("\n🚀 Training branch model...")
        model_branch = RandomForestClassifier(n_estimators=100, max_depth=20, random_state=42, n_jobs=-1)
